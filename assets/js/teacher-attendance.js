...
function renderQrDisplay(session, targetId = 'qr-result') {
  ...
  target.innerHTML = `<div class="teacher-token"><small>ACTIVE QR TOKEN</small><img id="attendance-qr" alt="Attendance QR code" src="${qrImageUrl(payload)}"><strong>${teacherEscape(session.token)}</strong><p>${teacherEscape(session.title || 'Attendance session')}</p><p>Expires <time datetime="${expiresAt.toISOString()}">${expiresAt.toLocaleTimeString()}</time></p><p id="qr-countdown"></p><div id="qr-image-error"></div></div>`;
...
async function createSession() {
  const subjects = await teacherApi('subjects', null, 'GET');
  const admt = subjects.subjects.find(subject => String(subject.code).trim().toUpperCase() === 'ADMT');
  const hasAdmt = Boolean(admt?.id);
  render(`<header><span class="eyebrow">Teacher</span><h1>Create QR session</h1></header><div class="card"><p class="notice">Generating a new session disables the previous active QR session.</p>${hasAdmt ? '' : message('ADMT is not configured in the backend. Add the ADMT subject before generating a QR session.', 'error')}<form id="session-form"><label>Session title<input name="title" value="Class attendance" required></label><label>Subject<select name="subject_id" required><option value="${hasAdmt ? teacherEscape(admt.id) : ''}" selected>${hasAdmt ? `${teacherEscape(admt.code)} — ${teacherEscape(admt.name)}` : 'ADMT — not configured'}</option></select></label><label>Valid for minutes<input name="minutes" type="number" min="1" max="240" value="10" required></label><button${hasAdmt ? '' : ' disabled'}>Generate QR</button></form><div id="qr-result"></div></div>`);
  document.querySelector('#session-form').onsubmit = async event => {
    ...
    try {
      ...
      const session = { token: data.token, qr_payload: data.qr_payload, expires_at: data.expires_at, title: Object.fromEntries(new FormData(event.target)).title };
...
async function initTeacherPage() {
  ...
  try {
    ...
    if (page === 'create') await createSession();
